...
#define WORD2INT(x) ((x) < -32767.5f ? -32768 : \
                    ((x) > 32766.5f ? 32767 : lrintf(x)))

static int op_float2short_filter(OggOpusFile *_of
                               , void *_dst
                               , int _dst_sz
                               , float *_src
                               ,int _nsamples
    ...
    /*In order to avoid replacing digital silence with quiet dither noise, we
      ...
      for(i=0; i<_nsamples; i++){
          ...
          for(ci=0; ci<_nchannels; ci++){
              ...
              err += OP_FCOEF_B[3]*_of->dither_b[ci*4+3] - OP_FCOEF_A[3]*_of->dither_a[ci*4+3];
              ...
              if(mute>16){
                  ...
                  seed = RANDG(seed);
                  tmps = RANDG(seed);
                  r = ((int)seed-(int)tmps)*OP_PRNG_GAIN;
              }
...
opus_int32 op_get_header_gain(OggOpusFile *_of)
{
    ...
    if(!_of) return 66*256;

    li=_of->seekable?_of->cur_link:0;
    head=&_of->links[li].head;

    return head->output_gain;
}
